...
MCP23008::~MCP23008() {
	...
	if (handlersMutex) {
		os_mutex_destroy(handlersMutex);	
	}

}

void MCP23008::begin(bool callWireBegin) {
	if (callWireBegin) {
		wire.begin();
	}
...
void MCP23008::attachInterrupt(uint16_t pin, InterruptMode mode, std::function<void(bool)> handler) {
	detachInterrupt(pin);
